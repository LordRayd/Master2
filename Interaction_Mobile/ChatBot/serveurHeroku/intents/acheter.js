...
function acheter(agent) {
    let texte = 'Je n\'ai malheureusement aucun vin qui corresponde à vos demandes';
    bouteilles = bdd_bouteilles[agent.parameters.couleur];
    if(agent.parameters.comparateur != undefined && agent.parameters.prix.amount != undefined){
        if(agent.parameters.comparateur == 'moins') {
            bouteille = bouteilles.filter( b => b.prix < agent.parameters.prix.amount);
            if(bouteille.length > 0){
                let indice = Math.floor(Math.random() * Math.floor(bouteille.length));
                let bouteille_choisi = bouteille[indice];
                texte = 'Je vous propose le "' + bouteille_choisi.nom +'".<br />';
                texte += 'Du producteur : ' + bouteille_choisi.domaine +'.<br />';
                texte += 'Pour un prix de : ' + bouteille_choisi.prix + ' euros.<br />';
                texte += 'Plus d\'informations <a href="' + bouteille_choisi.url + '">ici<a>';

            }
        } else if (agent.parameters.comparateur == 'plus'){
            bouteille = bouteilles.filter( b => b.prix > agent.parameters.prix.amount);
            if(bouteille.length > 0){
                let indice = Math.floor(Math.random() * Math.floor(bouteille.length));
                let bouteille_choisi = bouteille[indice];
        ...
        } else if (agent.parameters.comparateur == 'egale'){
            bouteille = bouteilles.filter( b => b.prix == agent.parameters.prix.amount);
            console.log(bouteille.length)
            if(bouteille.length > 0){
                let indice = Math.floor(Math.random() * Math.floor(bouteille.length));
                let bouteille_choisi = bouteille[indice];
                texte = 'Je vous propose le "' + bouteille_choisi.nom +'".<br />';
        ...
        let indice = Math.floor(Math.random() * Math.floor(bouteilles.length));
        let bouteille_choisi = bouteilles[indice];
        texte = 'Je vous propose le "' + bouteille_choisi.nom +'".<br />';
        texte += 'Du producteur : ' + bouteille_choisi.domaine +'.<br />';
        texte += 'Pour un prix de : ' + bouteille_choisi.prix + ' euros.<br />';
        texte += 'Plus d\'informations <a href="' + bouteille_choisi.url + '">ici<a>';
